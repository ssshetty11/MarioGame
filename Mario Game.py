...
class Dragon:
    ...
    def update(self):
        SCREEN.blit(self.dragon_img, self.dragon_img_rect)
...
def start_game():
    SCREEN.fill(BLACK)
    start_img = pygame.image.load('./start_game.png')
...
def game_loop():
    while True:
        ...
        while True:
            ...
            top_score_font = font.render('High Score:'+str(highscore.high_score),True,WHITE)
            ...
            SCREEN.blit(cactus_img, cactus_img_rect)
            SCREEN.blit(fire_img, fire_img_rect)
            mario.update()
            for f in flames_list:
